...
export const cancelJob = functions.https.onRequest(async (req, res) => {
  ...
  const { jobId, reason } = req.body as CancelJobRequest;
  ...
  try {
    ...
    const assignedDriverId = jobData?.driverId as string | undefined;
 ...
 */
async function notifyDriverOfCancellation(
  driverId: string,
  jobId: string,
  reason: string
): Promise<void> {
  ...
  try {
    ...
    const fcmToken = driverData.fcmToken as string;
