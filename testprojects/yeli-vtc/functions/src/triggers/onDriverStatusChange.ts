...
import { calculateDistanceKm, Location, VehicleCategory } from "../dispatch/dispatchAlgorithm";
...
/**
 * Pending job structure from Firestore
 */
...
export const onDriverStatusChange = functions.firestore
  ...
  .onUpdate(async (change, context) => {
    ...
    const beforeData = change.before.data() as Omit<Driver, "id">;
    const afterData = change.after.data() as Omit<Driver, "id">;
    ...
    try {
      ...
      let nearestJob: PendingJob | null = null;
      ...
      for (const doc of pendingJobsSnapshot.docs) {
        ...
        const distanceKm = calculateDistanceKm(afterData.location, job.pickupLocation);
